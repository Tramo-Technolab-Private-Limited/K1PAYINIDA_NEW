          ...
          if (Response.status == 200) {
            if (Response.data.code == 200) {
              ...
              setTransactionDetail([
                {
                  ...Response?.data?.txnId,
                  bal: Response.data.data.data.balanceAmount || null,
                },
              ]);
              handleCloseConfirmDetail();
              handleOpenResponse();
            ...
            if (Response.status == 200) {
              if (Response.data.code == 200) {
                ...
                setTransactionDetail([
                  {
                    ...Response?.data?.txnId,
                    bal: Response?.data?.data?.balanceAmount || null,
                  },
                ]);
                handleCloseConfirmDetail();
                handleOpenResponse();
    ...
    <>
      ...
      >
        <RoleBasedGuard hasContent roles={["agent"]}>
          ...
          {!user?.fingPayAPESRegistrationStatus ||
          ...
          <MotionModal open={openConfirmDetail} width={{ xs: "95%", md: 400 }}>
            <Box>
              ...
              <Stack flexDirection={"row"} gap={1} sx={{ mt: 2 }}>
                ...
                <Button
                  variant="outlined"
                  onClick={() => handleCloseConfirmDetail()}
                >
                  Close
                </Button>
              </Stack>
              ...
              >
                <Stack flexDirection={"column"} alignItems={"center"}>
                  <Typography variant="h3">
                    Transaction{" "}
                    {sentenceCase(transactionDetail[0]?.status || "")}
                  </Typography>
                  {failedMessage && (
